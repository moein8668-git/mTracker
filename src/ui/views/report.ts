...
export function viewReport(repo: Repo): string {
  ...
  for (const t of tasks) {
    ...
    const chart = state.chartType === 'line'
      ? lineChartHTML(a.days, [{ name: t.name, color: t.color, values: a.days.map(d => d.hours), taskId: t.id }],
    ...
    html += '<section class="card" style="--task:' + t.color + '">' +
      ...
      statBox('حد مجاز نوسان', fmtHours(a.sdLimit, s), 'ساعت') +
